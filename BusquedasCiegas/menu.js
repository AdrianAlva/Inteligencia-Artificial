...
controles = {
    ...
    funciones:{
        ...
        el_mejor_el_primero(){
            ...
            addChange(nodoInicio,"inicio")
            addChange(nodoFinal,"fin")
            graph.el_mejor_el_primero(nodoInicio,nodoFinal)
            addChange(nodoInicio,"inicio")
            addChange(nodoFinal,"fin")
        },
        _click(clickInfo){
            ...
            if(_clickStatus == "seleccionando_inicio"){
                ...
                /////
                let coordaux= this.id.split(',')
                coordaux[0]=Number(coordaux[0])
                coordaux[1]=Number(coordaux[1])
                
                /////
                graph.removeNode(_size*coordaux[0]+coordaux[1])
            }
        },
        porcentaje_update(){
...
document.getElementById(controles.buttons_id.el_mejor_el_primero).addEventListener("click", controles.funciones.el_mejor_el_primero)
